...
/**
 * Función interna base: construye y dispara el fetch al puente PHP.
 *
...
async function sendEmail(to, subject, html) {
  ...
  if (!response.ok) {
    ...
    throw new Error(errData.error || `Error en el puente PHP (Status ${response.status})`);
  }
...
function buildWelcomeCouponHtml(couponCode, discount) {
  return `
<!DOCTYPE html>
<html lang="es">
<head>
...
function buildPaymentVerifiedHtml(customerName, orderId) {
...
function buildOrderReadyHtml(customerName, orderId) {
  const shortId = getShortId(orderId);
  return `
<!DOCTYPE html>
<html lang="es">
...
export async function enviarCorreoPagoVerificado(correoCliente, nombreCliente, orderId) {
  try {
    const subject = '¡Tu pago ha sido verificado! - SRX Tech';
    const html = buildPaymentVerifiedHtml(nombreCliente, orderId);
    return await sendEmail(correoCliente, subject, html);
  } catch (error) {
...
export async function enviarCorreoPedidoListo(correoCliente, nombreCliente, orderId) {
  try {
    const subject = '¡Tu pedido por encargo está listo! - SRX Tech';
    const html = buildOrderReadyHtml(nombreCliente, orderId);
    return await sendEmail(correoCliente, subject, html);
  } catch (error) {
...
export async function sendCouponEmail(userEmail, couponCode, discountPercent) {
  try {
    const subject = '¡Tu regalo de bienvenida en SRX Tech está listo!';
    const html = buildWelcomeCouponHtml(couponCode, discountPercent);
 ...
 *
 * @param {Object} purchaseData - Datos detallados de la compra y productos
 * @returns {Promise<Object>} Respuesta { success: true, data } o lanza un error
 */
export async function enviarCorreoCompraExitosa(purchaseData) {
  try {
    ...
    if (!response.ok) {
      const errData = await response.json().catch(() => ({}));
      throw new Error(errData.error || `Error en el puente PHP de compra (Status ${response.status})`);
    }

    const data = await response.json();
    return { success: true, data };
  } catch (error) {
    console.error(`[emailService:enviarCorreoCompraExitosa] Error al enviar correo de compra:`, error);
    throw error;
  }
}
